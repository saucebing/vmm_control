...
def param_to_state(param):
    state = []
    for ind_res in range(0, NUM_RESOURCES):
        state.append(param[ind_res * (NUM_APPS - 1) + 0])
        state.append(param[ind_res * (NUM_APPS - 1) + 1])
        state.append(NUM_UNITS[ind_res] - param[ind_res * (NUM_APPS - 1) + 0] - param[ind_res * (NUM_APPS - 1) + 1])
    return state

def state_to_param(state):
    param = []
    for ind_res in range(0, NUM_RESOURCES):
        param.append(int(state[ind_res * NUM_APPS + 0]))
        param.append(int(state[ind_res * NUM_APPS + 1]))
    return param
